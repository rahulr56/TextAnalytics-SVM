...
try:
    data = pd.read_csv(open('/users/rrachapa/TextAnalytics-SVM/DataSet/DigitRecognition/optdigits_raining.csv'))
...
try:
    testdata = pd.read_csv(open('/users/rrachapa/TextAnalytics-SVM/DataSet/DigitRecognition/optdigits_test.csv'))
except Exception as e:
...
print "Cross Validation score : "+str(cvs(svm_map,X=crossValidationTrain, y=crossValidationTrainResults,cv=5))
print "Prediction rate is "+str(svm_map.score(testFeatures,testResults)*100)
